...
Rectangle.prototype.transform = function* transform(affineTransform) {
    for (let i = 0; i < 4; i++) {
        switch (i) {
            case 0:
                yield new Vector2D(...affine(this.x + this.width, this.y, affineTransform));
                break;
            case 1:
                yield new Vector2D(...affine(this.x, this.y, affineTransform));
                break;
            case 2:
                yield new Vector2D(...affine(this.x, this.y + this.height, affineTransform));
                break;
            case 3:
                yield new Vector2D(...affine(this.x + this.width, this.y + this.height, affineTransform));
                break;
            // }
        }
    }
