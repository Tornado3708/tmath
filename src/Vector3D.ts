...
import { HasX, HasY, HasZ, Vector3D, Vector3DConstructor } from "tmath";



const _vector3d: Vector3DConstructor = (...args): Vector3D => {
...
{ 
  const abstractAddition = (callback: (a: number, b: number) => number) => (a: Vector3D, b: Vector3D) => Vector3D(callback(a.x, b.x), callback(a.y, b.y), callback(a.z, b.z));
...
_vector3d.unit = ({x, y, z}: Vector3D) => {
  const inverse = 1 / Math.hypot(x, y, z);
  return _vector3d(x * inverse, y * inverse, z * inverse);
};

_vector3d.dot = (a: Vector3D, b: Vector3D): number => a.x * b.x + a.y + b.y + a.z * b.z;
